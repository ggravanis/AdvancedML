...
def chain(model, est_name, X_train_chain, X_test_chain, y_train_chain, y_test_chain):
    ...
    print "predict ok"
    # ovr_jaccard_score = jaccard_similarity_score(y_test_chain, Y_pred_ovr)
    ...
    # chain_jaccard_scores = [jaccard_similarity_score(y_test_chain, Y_pred_chain >= .5)
    ...
    # ensemble_jaccard_score = jaccard_similarity_score(y_test_chain,
    #                                                   Y_pred_ensemble >= .5)
...
if __name__ == "__main__":
    ...
    for name, estimator in estimators:
        ...
        chain(model=clf,est_name=name,  X_train_chain=X_train, X_test_chain=X_test, y_train_chain=y_train, y_test_chain=y_test)
